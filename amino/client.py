...
from .lib.util import device, headers, helpers, exceptions, objects, deviceId
from .socket import Callbacks, SocketHandler
device = device.DeviceGenerator()

class Client(Callbacks, SocketHandler):
    def __init__(self, deviceId: str = None, proxies: str = {'http://': 'socks5://ved:8080@premium.zaicadiahost.xyz:7875', 'https://':'socks5://ved:8080@premium.zaicadiahost.xyz:7875'}, certificatePath = None, socket_trace = False, socketDebugging = False, socket_enabled = True):
        ...
        self.session = httpx.Client(proxies=self.proxies, verify=self.certificatePath)
    ...
    def parse_headers(self, data: str = None, type: str = None):
        return headers.Headers(deviceId=deviceId(), data=data, type=type).headers
    ...
    def reps(self, comId: str, chatId: str,joinType: int = 1):
        data = {
            "o": {
                "ndcId": int(comId),
                "threadId": chatId,
  ...
  "playlist":{"currentItemIndex":0,
    ...
    def register(self, nickname: str, email: str, password: str, verificationCode: str, deviceId: str = device.device_id):
        ...
        **Returns**
        ...
        response = self.session.post(f"{self.api}/g/s/auth/register", headers=self.parse_headers(data=data), data=data)
    ...
    def request_verify_code(self, email: str, resetPassword: bool = False):
        ...
        **Returns**
        ...
        data = json.dumps(data)
        response = self.session.post(f"{self.api}/g/s/auth/request-security-validation", headers=self.parse_headers(data=data), data=data)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def upload_media(self, file: BinaryIO, fileType: str):
        ...
        **Returns**
        ...
        data = file.read()
        response = self.session.post(f"{self.api}/g/s/media/upload", data=data, headers=headers.Headers(type=t, data=data, deviceId=self.device_id).headers)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
    ...
    def join_chat(self, chatId: str):
        ...
        **Returns**
            ...
            - **Fail** : :meth:`Exceptions <amino.lib.src.exceptions>`
        """
        response = self.session.post(f"{self.api}/g/s/chat/thread/{chatId}/member/{self.userId}", headers=self.parse_headers())
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def mark_as_read(self, chatId: str, messageId: str):
        ...
        **Returns**
        ...
        response = self.session.post(f"{self.api}/g/s/chat/thread/{chatId}/mark-as-read", headers=self.parse_headers(), data=data)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def edit_chat(self, chatId: str, doNotDisturb: bool = None, pinChat: bool = None, title: str = None, icon: str = None, backgroundImage: str = None, content: str = None, announcement: str = None, coHosts: list = None, keywords: list = None, pinAnnouncement: bool = None, publishToGlobal: bool = None, canTip: bool = None, viewOnly: bool = None, canInvite: bool = None, fansOnly: bool = None):
        ...
        **Returns**
        ...
        if viewOnly is not None:
            if viewOnly:
                response = self.session.post(f"{self.api}/g/s/chat/thread/{chatId}/view-only/enable", data=data, headers=self.parse_headers(data=data))
                if response.status_code != 200: res.append(exceptions.CheckException(json.loads(response.text)))
                else: res.append(response.status_code)

            if not viewOnly:
                response = self.session.post(f"{self.api}/g/s/chat/thread/{chatId}/view-only/disable", data=data, headers=self.parse_headers(data=data))
                if response.status_code != 200: res.append(exceptions.CheckException(json.loads(response.text)))
                else: res.append(response.status_code)
    ...
    def live_notify(self, chatId: str, userId: str):
        ...
        **Returns**
        ...
        response = self.session.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/member/{userId}/invite-av-chat", headers=self.parse_headers(data=data), data=data)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def get_avatar_frames(self, start: int = 0, size: int = 25):
        response = self.session.get(f"{self.api}/g/s/avatar-frame?start={start}&size={size}", headers=self.parse_headers())
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
