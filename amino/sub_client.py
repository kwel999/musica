...
from . import client
from .lib.util import device, headers, exceptions, objects, deviceId
...
class SubClient(client.Client):
    ...
    def send_message(self, chatId: str, message: str = None, messageType: int = 0, file: BinaryIO = None, fileType: str = None, replyTo: str = None, mentionUserIds: list = None, stickerId: str = None, embedId: str = None, embedType: int = None, embedLink: str = None, embedTitle: str = None, embedContent: str = None, embedImage: BinaryIO = None, linkSnippet: str = None, linkSnippetImage: BinaryIO = None):
        ...
        **Returns**
        ...
        data = json.dumps(data)
        response = self.session.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/message", headers=headers.Headers().headers, data=data)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def accept_host(self, chatId: str, requestId: str):
        ...
        response = self.session.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/transfer-organizer/{requestId}/accept", headers=headers.Headers().headers)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def kick(self, userId: str, chatId: str, allowRejoin: bool = True):
        ...
        response = self.session.delete(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/member/{userId}?allowRejoin={allowRejoin}", headers=headers.Headers().headers)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def join_chat(self, chatId: str):
        ...
        **Returns**
            ...
            - **Fail** : :meth:`Exceptions <amino.lib.src.exceptions>`
        """
        response = self.session.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/member/{self.profile.userId}", headers=self.parse_headers())
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def feature(self, time: int, userId: str = None, chatId: str = None, blogId: str = None, wikiId: str = None):
        if chatId:
            ...
            elif time == 3: time = 10800
            else: raise exceptions.WrongType(time)
        
        elif blogId or wikiId:
    ...
    def get_chat_lurkers(self, start: int = 0, size: int = 100):
        response = self.session.get(f"{self.api}/x{self.comId}/s/live-layer/public-chats?start={start}&size={size}", headers=self.parse_headers())
